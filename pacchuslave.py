...
from time import *
from random import *
import time,subprocess,discord,os,json,io,urllib3,asyncio,youtube_dl
from jikanpy import Jikan
import numpy as np
from discord.ext import commands

#deprecating _count_statistics
...
async def on_ready():
    ...
    statustxt = "Bug fixes in progress"
...
async def help(ctx):   
    ...
    embed.add_field(name="How can I help you", value=f"{self_name} commands", inline=False)
    embed.add_field(name=f"{command_prefix}anime", value="Searches for given anime", inline=True)
    embed.add_field(name=f"{command_prefix}manga", value="Searches for give Manga", inline=True)
    embed.add_field(name=f"{command_prefix}anichar", value="Searches for given Anime Charactor ", inline=True)
    embed.add_field(name=f"{command_prefix}anipics", value="Searches for Images of given Anime Charactor", inline=True)
    embed.add_field(name=f"{command_prefix}avatar @mention", value="Steals the person's DP :d", inline=False)
    embed.add_field(name=f"{command_prefix}stats", value="disabled **bugs**", inline=False)
    embed.add_field(name=f"{command_prefix}help", value="isnt it obvious :o", inline=False)
    embed.add_field(name=f"{command_prefix}perk", value="Extra stuff run the command for more commands", inline=False)
...
async def perk(ctx):
    ...
    embed.add_field(name="Ping me?", value=f"I Know when you ping me", inline=False)
    embed.add_field(name=f"{command_prefix}kill @mention", value=f"Kills the user ... well not really", inline=False)
    embed.add_field(name=f"{command_prefix}kiss @mention", value=f"Kisses the user? not a good proposal material.", inline=False)
    embed.add_field(name=f"{command_prefix}hug @mention", value=f"Hugs the user?", inline=False)
    embed.add_field(name=f"{command_prefix}avatar @mention", value=f"Something of use atleast", inline=False)
    embed.add_field(name=f"{command_prefix}bruh", value=f"{command_prefix} bruh [image/text]", inline=False)        
    embed.add_field(name=f"{command_prefix}irumachi", value="Sends an Adorable photo of irumakun from Marimashita irumakun", inline=False)
...
async def avatar(ctx,member:discord.Member):
    ...
    if(ctx.message.author == hgp  or hgp == None):
        ...
        embed = discord.Embed(title="Beep Boop",description=f"{ctx.message.author.mention} steals {hgp.mention}'s profile pic 👀'", colour=discord.Colour(0x00ffb7))
...
async def kiss(ctx,member:discord.Member):
    ...
    await ctx.message.add_reaction('👄')
    if(ctx.message.author == hgp  or hgp == None):
        embed = discord.Embed(title=" ",description=f"{ctx.message.author.mention} kisses themselves..HOW!!!?", colour=discord.Colour(0x00ffb7))
        embed.set_image(url=choice(perks['links']['erotic_perv']))
    else:
        embed = discord.Embed(title="💋",description=f"{ctx.message.author.mention} kisses {hgp.mention}", colour=discord.Colour(0x00ffb7))
...
async def bruh(ctx,link=None):
    ...
    if(link == None):
        try:
            await ctx.reply(serverlist[str(ctx.guild.id)]['bruh'])
        except KeyError:
...
async def stats(ctx):
    ...
    try:
        ...
        embed.set_footer(text=f"Facebook doesnt sponsor this data", icon_url=self_avatar)
        await ctx.send(embed=embed)
    except KeyError:
        __initiate_default_stats__(serverlist,ctx.guild.id)
        embed.add_field(name="Database API cannot be reachable", value="404?", inline=True)
        embed.set_footer(text=f"data is volatile", icon_url=self_avatar)
        await ctx.send(embed=embed)
